          ...
          {#if selectedWorktree.head.upstream}
            {#if selectedWorktree.head.upstream.ahead > 0 || selectedWorktree.head.upstream.behind > 0}
              <span class="sync-indicator">
                {#if selectedWorktree.head.upstream.ahead > 0}↑{selectedWorktree
                    .head.upstream.ahead}{/if}
                {#if selectedWorktree.head.upstream.behind > 0}↓{selectedWorktree
                    .head.upstream.behind}{/if}
              </span>
            {/if}
          {/if}
        </span>
    ...
    <div class="dropdown-panel">
      ...
      <div class="worktree-list">
        {#each worktrees as worktree (worktree.path)}
          ...
          {@const wtHasChanges = worktree.status
            ? !worktree.status.is_clean
            : false}
            ...
            >
              ...
              <div class="worktree-info">
                ...
                <div class="option-branch-line">
                  <span class="option-branch"
                    >{worktree.head.branch ?? "detached"}</span
                  >
                  {#if worktree.head.upstream}
                    <span class="option-upstream"
                      >→ {worktree.head.upstream.remote_branch}</span
                    >
                    {#if worktree.head.upstream.ahead > 0 || worktree.head.upstream.behind > 0}
                      <span class="option-sync">
                        {#if worktree.head.upstream.ahead > 0}↑{worktree.head
                            .upstream.ahead}{/if}
                        {#if worktree.head.upstream.behind > 0}↓{worktree.head
                            .upstream.behind}{/if}
                      </span>
                    {/if}
                  {/if}
              ...
              <div class="worktree-meta">
                {#if wtHasChanges}
                  <span class="change-indicator" title="Has uncommitted changes"
                  ></span>
                {/if}
                <span class="worktree-time"
                  >{formatRelativeTime(worktree.last_commit_timestamp)}</span
                >
              </div>
            </button>
            {#if !worktree.is_main}
